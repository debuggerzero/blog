  ...
  <div style="width: 100%">
    ...
    >
      ...
      <el-table-column label="操作" fixed="right" width="400">
        <template #default="scope">
          <el-button-group>
            <el-button type="primary" plain @click="goToEditArticle(scope.row)">
              <el-icon class="el-icon--left">
                <Edit/>
              </el-icon>
              编辑
            </el-button>
            <el-button type="primary" plain @click="open(scope.row.id)">
              <el-icon class="el-icon--left">
                <Delete/>
              </el-icon>
              删除
...
const getTotalCount = async () => {
  const count = await proxy.Request({
    url: globalRouter.backEndRouter.BACK_END_GET_TOTAL_ARTICLE_COUNT + '/' + userInfo.id + '/0',
    methods: 'GET',
  })
...
const deleteArticles = async (id) => {
  const results = await proxy.Request({
    url: globalRouter.backEndRouter.BACK_END_DELETE_ARTICLE + '/' + id,
    methods: 'DELETE',
  })
  if (!results) return;
  init();
